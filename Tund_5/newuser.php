  ...
  if(isset($_POST["submitUserData"])) {
	  ...
	  if(!empty($birthDay) and !empty($birthMonth) and !empty($birthYear)) {
		  if(checkdate($birthMonth, $birthDay, $birthYear)){
			  ...
			  $birthDateError = "Valitud kuupäeva pole olemas";
		  }
	  ...
	  //paroolikontroll (kas olemas/kas 8 tähte/2x ühte moodi)
	  //strlen($_POST["password]) > 8
	  
	  //kui kõik korras, salvestada
	  
	  if(empty($nameError) and empty($surnameError) and empty($birthMonthError) and empty($birthYearError) and empty($birthDayError) and empty($birthDateError) and empty($genderError) and empty($emailError) and empty($passwordError) and ($confirmpasswordError)) {
